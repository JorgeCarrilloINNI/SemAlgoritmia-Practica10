...
void MainWindow::on_graficarGrafos_clicked()
{
    int x = 0, y = 0;
    list<Neurona>& lista = listaNeuronas.listaNeurona();
    QGraphicsScene* g = new QGraphicsScene;
    ...
    for (Neurona &neurona : lista) {
        ...
        QBrush brush(color);
        g->addEllipse(neurona.getPosX(), origen, 5, 5, pen, brush);
        g->addSimpleText(QString::number(neurona.getId()))->setPos(neurona.getPosX(), origen + 10);
        FuerzaBrutaBuscarCercania(x, y, lista, neurona);
        qreal origen2 = alto - y;
        g->addLine(neurona.getPosX(), origen, x-5, origen2-5, pen);

        qreal midX = (neurona.getPosX() + x)/2;
        qreal midY = alto - ((neurona.getPosY() + y)/2);

        QRectF cuadro(x-20 /2, origen2-20 /2, 10, 10);
        g->addSimpleText(QString::number(neurona.getVoltaje()))->setPos(midX-5, midY-5);
        g->addRect(cuadro, pen, brush);
    }
}
